...
int main(int argc, char *argv[])
{
  ...
  detect_faces(g_ort, env, session_options, input_file, &bounding_boxes, &bounding_boxes_row, &bounding_boxes_col, &face_scores, &face_scores_length, &face_landmarks_5, &face_landmarks_5_row, &face_landmarks_5_col);
  qDebug() << bounding_boxes_row << bounding_boxes_col << face_scores_length << face_landmarks_5_row << face_landmarks_5_col;
  free(bounding_boxes);
  free(face_scores);
